...
static int fb_open (FBInfo *fb, const char *fbfilename)
{
    ...
    fail: printf("%s is not a framebuffer.\n", fbfilename);
    close(fb->fd);
...
static int snap2png (const char *filename, int quality, FBInfo *fb)
{
    ...
    for (; i < height; i++) {
        ...
        for (j = 0; j < width; j++, line += fb_bpp(fb)) {
            ...
            fb->unpack(fb, line, image_buffer + offset,
                    image_buffer + offset + 1, image_buffer + offset + 2);
        }
...
static int snap2jpg (const char *filename, int quality, FBInfo *fb)
{
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        int i = 0;
        int offset = 0;
        unsigned char* line = fb->bits
                + cinfo.next_scanline * fb_width(fb) * fb_bpp(fb);
